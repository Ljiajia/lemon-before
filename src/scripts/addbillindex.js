require(["./config"], function() {
    require(["muimin", "swiper", "picker"], function(muimin, Swiper, picker) {
        ...
        var dtPickerM = new mui.DtPicker({ type: "date" }); //传参data    可以选择  年月日

        function dtPickerShow(dtPicker, type) { //设置选择的年月
        ...
        function tab() { //tab切换
            for (let i = 0; i < tabs_li.length; i++) {
                tabs_li[i].onclick = function() {
                    moneytype = tabs_li[i].innerHTML; //获取当前页面是支出还是收入还是转账
        ...
        function ounter() { //计算器
            mui(footlist).on("tap", "span", function() {
                if (this.innerText == "x") { //判断点击的是x
                    if (money.value == "0.00") { //如果金额内容为0.00，那就不删除
                        money.value = "0.00";
                    } else {
                        money.value = money.value.substr(0, money.value.length - 1); //否则每次删除一位
                    }
                } else if (money.value == "0.00") { //如果金额为初始的0.00，就把计算器输入的值赋给总金额
                    money.value = this.innerText;
                } else if (money.value.indexOf(".") != -1 && this.value == ".") { //设置只能输入以为小数点
                    money.value = money.value;
                } else if (money.value.indexOf(".") != -1 && money.value.split(".")[1].length == "2") { //保留小数点后2位
                    money.value = money.value;
                } else {
                    money.value += this.innerText;
                }
        ...
        function selectIcon() { //选择icon
            mui(swiperslide).on("tap", "dl", function() { //选择icon的时候获取到需要传的参数
                for (let i = 0; i < swiperslide.length; i++) { //排他法，让其他的dl>dt 不添加cur类名
                    for (var j = 0; j < swiperslide[i].children.length; j++) {
                        swiperslide[i].children[j].firstElementChild.classList.remove("cur");
                    }
                }
                ...
                if (icon == "mui-icon mui-icon-plusempty") { //判断如果点击的是添加按钮，就跳转到添加到页面
                    location.href = `iconlist.html?type=${moneytype}`;
                }
            });
            achieve.addEventListener("tap", function() { //点击完成，发起ajax请求，把需要的参数传过去
        ...
        function addEvent() { //放所有的点击事件
            back.addEventListener("tap", function() { //点击返回，跳回首页面
                window.location.href = "/"
            })
            mui(nowdata).on("tap", "span", function() { //点击的时候显示日期组件
                dtPickerShow(dtPickerM, "date");
            })

        }

        function addbill(uid, icon, moneytype, timer, moneys, cid) { //添加账单
        ...
        function renderIcon() { //渲染支出，收入，转账页面
                ...
                success: function(data) {
                    if (data.code == 1) {
                        ...
                        data.msg.forEach(function(item, index) {
                            ...
                            if (item.type == "支出") {
                                onestr += `<dl><dd class="${newclass}"><span class="${item.icon}"></span></dd><dt>${item.cid}</dt></dl>`
                            } else if (item.type == "收入") {
                                twostr += `<dl><dd class="${newclass}"><span class="${item.icon}"></span></dd><dt>${item.cid}</dt></dl>`
                            } else if (item.type == "转账") {
                                threestr += `<dl><dd class="${newclass}"><span class="${item.icon}"></span></dd><dt>${item.cid}</dt></dl>`
                            };
                        })
