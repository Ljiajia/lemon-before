require(["./config"], function() {
    require(["muimin", "swiper"], function(muimin, swiper) {
        ...
        function rendericonAll() { //渲染所有的icon
                ...
                success: function(data) {
                    if (data.code == 1) {
                        ...
                        data.msg.forEach(function(item) {
                            str += `<li><span class="${item.icon}"></span></li>`;
                        })
        ...
        function addEvent() { //绑定事件
            mui(ullist).on("tap", "span", function() { //点击所有的icon
                currentname.className = this.className; //获取点击的icon图标 赋值给默认的选项
                save.addEventListener("tap", function() { //点击保存，判断分类是否为空，若为空，不能提交
            ...
            back.addEventListener("tap", function() { //点击返回，跳回页面
